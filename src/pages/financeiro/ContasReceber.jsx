...
export default function ContasReceber() {
  ...
  function calcularResumo(dados) {
    ...
    setResumo({ vencidos: v, hoje: h, aReceber: ar, recebido: r, total: t });
  }
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-xl font-bold text-gray-700">Contas a Receber</h2>
        <button
          onClick={handleNovaReceita}
          className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 shadow-md transition-all text-sm"
        >
      ...
      <div className="flex gap-3 mb-6">
        <div className="flex-1 bg-gray-50 rounded-lg flex items-center px-4 border border-gray-200 focus-within:border-blue-400 transition-colors">
          ...
          <input
            type="text"
            placeholder="Pesquisar cliente, descrição..."
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-50">
            <tr className="border-b border-gray-200 text-xs font-bold text-gray-500 uppercase tracking-wider">
              ...
              <th className="p-4 w-32">Recebimento</th>
                  ...
                  {group.items.map((item) => (
                    <React.Fragment key={item.id}>
                      <tr
                        ...
                        className={`border-b border-gray-50 hover:bg-green-50/30 transition-colors group cursor-pointer ${
                          expandedRows[item.id] ? "bg-green-50/50" : ""
                        }`}
                      >
                        ...
                        <td className="p-4">
                          <div className="flex justify-center gap-1">
                            <button
                              ...
                              title={
                                item.status === "pago" ? "Reabrir" : "Receber"
                              }
                            >
                              {item.status === "pago" ? (
                                <XCircle size={16} />
                          ...
                          <td colSpan="7" className="p-4 px-8">
                            <div className="grid grid-cols-4 gap-6 text-sm">
                              ...
                              <div className="col-span-1">
                                {item.anexo_url &&
                                JSON.parse(item.anexo_url || "[]").length >
                                  0 ? (
                                  <span className="flex items-center gap-2 text-blue-600 mt-2">
                                    <Paperclip size={14} />{" "}
                                    {JSON.parse(item.anexo_url).length} Anexos
                                  </span>
                                ) : (
      ...
      {modalOpen && (
        <NovaReceitaModal
          ...
          receitaParaEditar={itemParaEditar}
          onSuccess={() => {
            fetchTransacoes();
          }}
        />
...
function StatusBadge({ status }) {
  ...
  const labels = { pago: "Recebido", pendente: "A Receber" };
  return (
    <span
